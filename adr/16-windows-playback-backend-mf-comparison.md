...
- ~~PCM 境界を持つ方式であれば ASIO へ到達できることを、現在の出力デバイス切り替え実装の方針として扱います。~~\
  (2026-06-20 17:40) この読み方は取り下げました。この ADR はバックエンド方式比較の記録であり、現行の Windows 出力デバイス切り替えは ADR-19 の通り WASAPI 限定です。
- この ADR にある ASIO への言及は、PCM を扱うアーキテクチャ上の可能性を述べたものです。ASIO runtime integration の実装指針、API 形状、UI 方針として参照しないでください。
