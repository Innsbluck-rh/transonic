...
pub trait PlaybackBackend: Send {
    fn load(&mut self, request: PreparedBinaryRequest, autoplay: bool) -> Result<(), String>;
...
impl PlaybackBackend for UnsupportedPlaybackBackend {
    fn load(&mut self, _request: PreparedBinaryRequest, _autoplay: bool) -> Result<(), String> {
    ...
    while let Ok(job) = rx.recv() {
        match job {
            PlaybackJob::Load {
                ...
                let _ = result_tx.send(worker.load(request, autoplay));
            }
