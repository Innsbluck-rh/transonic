...
fn configure_libclang_path() {
    ...
    for candidate in libclang_path_candidates() {
        if candidate.join("libclang.dll").is_file() || candidate.join("clang.dll").is_file() {
            env::set_var("LIBCLANG_PATH", &candidate);
            println!("LIBCLANG_PATH is set at {}", candidate.display());
            return;
        }
    }
}
...
fn libclang_path_candidates() -> Vec<PathBuf> {
    ...
    for env_key in ["NDK_HOME", "ANDROID_NDK_HOME"] {
    ...
    if let Some(path) = env::var_os("ANDROID_HOME") {
        let ndk_dir = PathBuf::from(path).join("ndk");
